...
- [Variables](<#variables>)
- [func buildStorageArgs\(storage string\) \[\]string](<#buildStorageArgs>)
...
<a name="ControllerJob"></a>ControllerJob is the job for the storage controller.
...
<a name="buildStorageArgs"></a>
## func [buildStorageArgs](<https://github.com/ecshreve/slomad/blob/main/internal/registry/storage.go#L26>)
...
Currently this is running in monolith mode, so the storage controller and node are running on the same host.
